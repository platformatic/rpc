...
async function generateRpcSchemaCommand (argv) {
  ...
  if (args.help) {
    const help = await readFile(resolve(import.meta.dirname, 'help.txt'), 'utf8')
  ...
  if (!isAbsolute(tsConfigPath)) {
    tsConfigPath = resolve(process.cwd(), tsConfigPath)
  }
  ...
  if (!isAbsolute(openapiSchemaPath)) {
    openapiSchemaPath = resolve(process.cwd(), openapiSchemaPath)
  }
...
if (import.meta.main) {
  await generateRpcSchemaCommand(process.argv.slice(2))
}
